...
public class NShell {
    ...
    private static final String EXIT = "exit" + NShell.EOL;
            ...
            try {
                ...
                if((line = reader.readLine()) != null) {
                    ...
                    while((line = reader.readLine()) != null) {
                        mBuffer.append(NShell.EOL).append(line);
                    }
                }
            } catch(IOException e) {
                e.printStackTrace();
    ...
    private static String suExec(String cmd) throws ShellException {
        try {
            ...
            shell.writeBytes(cmd + NShell.EOL);
            shell.flush();
            shell.writeBytes(NShell.EXIT);
            shell.flush();
            proc.waitFor();
            return buffer.getOutput();
    ...
    private static boolean isRootUid() throws ShellException {
        for(UID_COMMAND uid : UID_COMMAND.values()) {
            String output = NShell.sudo(uid.getCommand());
