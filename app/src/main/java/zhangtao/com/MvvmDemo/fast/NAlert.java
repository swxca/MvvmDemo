...
import zhangtao.com.MvvmDemo.utils.ActivityCollector;

public class NAlert {
    // 有一个ok的阻塞alert,请不要在主线程调用
    // 在用户点击ok后返回
    public static void AlertOkSync(final String message, final String ButtonMsg, final Context context,final Boolean isForcedFinishApp){
        AlertOkCancelSync(message,ButtonMsg,null,context,isForcedFinishApp);
    }

    // 返回1 表示用户点了Ok(OkButtonText),返回2表示用户点了Cancel(CancelButtonText)
    // OkButtonText/CancelButtonText 传入null 表示不显示这个按钮
    // 请不要在主线程调用
    public static int AlertOkCancelSync(final String message, final String OkButtonText, final String CancelButtonText, final Context context, final Boolean isForcedFinishApp){
        final NSignalChan chan = NSignalChan.New();
